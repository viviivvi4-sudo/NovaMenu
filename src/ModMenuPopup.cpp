...
bool ModMenuPopup::init() {
    ...
    float btnWidth = 82.f;
    float startX = m_mainLayer->getContentSize().width / 2 - (btnWidth * 1.5f) - 6.f;
    float y = m_mainLayer->getContentSize().height - 38.f;
    ...
    m_contentLayer->setPosition({
        ...
        m_mainLayer->getContentSize().height / 2 - 10.f
    });
...
void ModMenuPopup::buildLevelTab() {
    auto info = CCLabelBMFont::create("Level Tools", "bigFont.fnt");
    info->setScale(0.5f);
    info->setPosition({0.f, 130.f});
    m_contentLayer->addChild(info);

    auto menu = CCMenu::create();
    menu->setPosition({0.f, 0.f});

    addStyledButton(menu, "Restart Level", 200, 0.f, 95.f, 180.f, 30.f);
    addStyledButton(menu, "Show Level Info", 201, 0.f, 58.f, 180.f, 30.f);

    m_contentLayer->addChild(menu);

    auto perfTitle = CCLabelBMFont::create("Performance bypass", "bigFont.fnt");
    perfTitle->setScale(0.38f);
    perfTitle->setPosition({0.f, 25.f});
...
void ModMenuPopup::buildPlayerTab() {
    auto info = CCLabelBMFont::create("Player Tools", "bigFont.fnt");
    info->setScale(0.5f);
    info->setPosition({0.f, 130.f});
    m_contentLayer->addChild(info);
    ...
    m_speedhackToggle = addToggleRow(menu, "Speedhack Enabled",
        menu_selector(ModMenuPopup::onSpeedhackToggle), 20.f, 90.f, g_speedhackEnabled);

    addNumberInput("Speed (0.01x-100x)", 130.f, 90.f, 80.f, g_speedhackValue, 0.01f, 100.f,
        [](float v) {
            g_speedhackValue = v;
        });

    m_speedhackMusicToggle = addToggleRow(menu, "Sync Music Speed",
        menu_selector(ModMenuPopup::onSpeedhackMusicToggle), 20.f, 55.f, g_speedhackMusicEnabled);

    m_noclipToggle = addToggleRow(menu, "Noclip (godmode)",
        menu_selector(ModMenuPopup::onNoclipToggle), 20.f, 20.f, g_noclipEnabled);

    m_safeModeToggle = addToggleRow(menu, "Safe Mode",
        menu_selector(ModMenuPopup::onSafeModeToggle), 20.f, -10.f, g_safeModeEnabled);

    m_autoSafeModeToggle = addToggleRow(menu, "Auto Safe Mode",
        menu_selector(ModMenuPopup::onAutoSafeModeToggle), 20.f, -35.f, g_autoSafeModeEnabled);
...
void ModMenuPopup::buildCosmeticTab() {
    auto info = CCLabelBMFont::create("Cosmetic Tools", "bigFont.fnt");
    info->setScale(0.5f);
    info->setPosition({0.f, 120.f});
    m_contentLayer->addChild(info);

    auto menu = CCMenu::create();
    menu->setPosition({0.f, 0.f});

    addStyledButton(menu, "Random Player Color", 220, 0.f, 90.f, 220.f, 30.f);
    addStyledButton(menu, "Reset Player Color", 221, 0.f, 55.f, 220.f, 30.f);

    m_disableTrailsToggle = addToggleRow(menu, "Disable All Trails",
        menu_selector(ModMenuPopup::onDisableTrailsToggle), 20.f, 15.f, g_disableTrails);

    m_disableParticlesToggle = addToggleRow(menu, "Disable Particles",
        menu_selector(ModMenuPopup::onDisableParticlesToggle), 20.f, -10.f, g_disableParticles);

    m_disableEffectsToggle = addToggleRow(menu, "Disable Effects",
        menu_selector(ModMenuPopup::onDisableEffectsToggle), 20.f, -35.f, g_disableEffects);
...
void ModMenuPopup::buildBotReplayTab() {
    ...
    stateLabel->setPosition({0.f, 95.f});
    ...
    tip->setPosition({0.f, 65.f});
    m_contentLayer->addChild(tip);

    auto radioMenu = CCMenu::create();
    radioMenu->setPosition({0.f, 0.f});
    m_recordRadio = addToggleRow(radioMenu, "Record", menu_selector(ModMenuPopup::onRecordRadio), 130.f, 35.f, br->isRecording());
    m_playRadio   = addToggleRow(radioMenu, "Play",   menu_selector(ModMenuPopup::onPlayRadio),   130.f, 10.f, br->isPlaying());
    ...
    m_exportNameInput->setPosition({0.f, -95.f});
    ...
    addStyledButton(menu, "Stop", 101, -100.f, -25.f, bw, bh);
    addStyledButton(menu, "Reset", 103, 0.f, -25.f, bw, bh);
    addStyledButton(menu, "Export", 104, -100.f, -60.f, bw, bh);
    addStyledButton(menu, "Import", 105, 0.f, -60.f, bw, bh);
