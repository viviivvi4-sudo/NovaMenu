...
class $modify(BRPlayLayer, PlayLayer) {
    ...
    void resetLevel() {
        ...
        // If you die mid-recording, the level restarts from frame 0 -
        // whatever was captured up to that point no longer lines up with
        // a fresh attempt. Rather than throwing it away, just stop
        // recording here (keeping what you already have) so you can
        // review/export it, or hit Record again yourself to redo the run.
        if (BotReplay::get()->isRecording()) {
            BotReplay::get()->stopRecording();
        }
    ...
    // Practice Mode calls THIS (not resetLevel) when you die and respawn
    // to a checkpoint instead of restarting the whole level. Without this
    // hook, recording had no idea the level had "jumped back" to a
    // checkpoint mid-run, so it kept appending input as if it was still
    // one continuous run - producing the exact "dies somewhere weird on
    // playback" bug. Same fix as resetLevel: just stop recording here
    // rather than discarding what's already captured.
